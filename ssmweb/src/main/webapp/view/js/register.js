...
(function ($) {
    ...
    function require(options){
        var rule={//验证规则
            phone:/^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/,
...
$("#randCode").blur(function () {
    $.ajax({
        type:'get',
        url:rootPath+'/verify/check?randCode='+$("#randCode").val(),
        cache:false,
        dataType:'json',
        success:function(data){
            if (data.result == "success") {
                $("#btn_part1").attr("disabled", false);
                $("#randCode").parent().children().eq(3).removeClass("hide");
            } else {
                $("#randCode").parent().addClass("v_error");
                $("#randCode").parent().children().eq(3).addClass("hide");
                $("#randCode").parent().children().eq(5).addClass("error");
                $("#randCode").parent().children().eq(5).text(data.result);
...
$("#randCode").focus(function () {
    $("#btn_part1").attr("disabled", false);
    $("#randCode").parent().children().eq(3).addClass("hide");
    $("#randCode").parent().children().eq(5).text("");
})

// 点击换一张验证码
function changeImg() {
    var imgSrc = $("#verifyImage");
